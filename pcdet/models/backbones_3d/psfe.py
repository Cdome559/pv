...
class PointNet2MSG(nn.Module):
    ...
    def forward(self, batch_dict):
        ...
        for i in range(len(self.SA_modules)):
            ...
            li_features = self.PA_modules[i](li_features)
            l_xyz.append(li_xyz)
        ...
        batch_dict['point_features'] = point_features.view(-1, point_features.shape[-1])  #(batch*16384, 128)
        batch_dict['point_coords'] = torch.cat((batch_idx[:, None].float(), l_xyz[0].view(-1, 3)), dim=1)  #(batch*16384, batch_idx, x, y, z)
        return batch_dict
