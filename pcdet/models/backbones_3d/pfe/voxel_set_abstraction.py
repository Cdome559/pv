    ...
    def forward(self, batch_dict):
        ...
        if 'bev' in self.model_cfg.FEATURES_SOURCE:
            ...
            point_bev_features = self.interpolate_from_bev_features(
                ...
                #将高度堆叠后的bev特征图spatial_features
                bev_stride=batch_dict['spatial_features_stride'] #高度堆叠后的bev特征图下采样倍数，默认8倍
            )
            point_features_list.append(point_bev_features) #将插值得到的关键点特征存入列表

        batch_size = batch_dict['batch_size'] 

        new_xyz = keypoints[:, 1:4].contiguous() #取出关键点xyz坐标
        new_xyz_batch_cnt = new_xyz.new_zeros(batch_size).int() #存储每帧点云关键点个数
        #获取batch每帧点云的关键点个数
        for k in range(batch_size):
            new_xyz_batch_cnt[k] = (keypoints[:, 0] == k).sum()
        """
        =================
        Extended VSA 中对原始点云的操作
        =================
        配置文件中
        ...
        """
        if 'raw_points' in self.model_cfg.FEATURES_SOURCE:
            raw_points = batch_dict['points'] #得到一批数据中所有原始点云数据 shape （N，5） batch_id, x, y, z, r
            #pooled_features( 2048*batch, 32)
            pooled_features = self.aggregate_keypoint_features_from_one_source(
                batch_size=batch_size, aggregate_func=self.SA_rawpoints, #聚合函数采用SA模块
                xyz=raw_points[:, 1:4], #原始点的xyz数据
                xyz_features=raw_points[:, 4:].contiguous() if raw_points.shape[1] > 4 else None, #取第五列反射强度
                xyz_bs_idxs=raw_points[:, 0], #每个点在batch中的索引
        ...
        for k, src_name in enumerate(self.SA_layer_names):  
            #获取非空voxel的xyz坐标index shape:( batch*16000, 4) 4->( batch_idx, z, y, x)
            cur_coords = batch_dict['multi_scale_3d_features'][src_name].indices    
            cur_features = batch_dict['multi_scale_3d_features'][src_name].features.contiguous()  #获取非空体素特征
            #获取每个非空体素在点云坐标中的xyz的中心点坐标 （ batch*16000, 3）
            ...
            #VSA操作
            pooled_features = self.aggregate_keypoint_features_from_one_source(
                batch_size=batch_size, aggregate_func=self.SA_layers[k],  #第k层sa网络
                #xyz获取所有体素在点云坐标系的中心点坐标，xyz_features获取体素在第k层的特征，xyz_bs_idxs所有点在batch中的mask
            ...
            point_features_list.append(pooled_features)  #得到的pooled_features，维度(2048*batch, C)
            '''
            =========================
            (2048*batch, 256) BEV视角下的点特征数据
            (2048*batch, 32) 原始点云的特征数据
            (2048*batch, 32) x_conv1第一次稀疏卷积后的特征
            (2048*batch, 64) x_conv2第二次稀疏卷积后的特征
            (2048*batch, 128) x_conv3第三次稀疏卷积后的特征
            (2048*batch, 128) x_conv4第四次稀疏卷积后的特征
            ==========================
            '''

        point_features = torch.cat(point_features_list, dim=-1)  #(2048*batch, 640)

        batch_dict['point_features_before_fusion'] = point_features.view(-1, point_features.shape[-1])
        point_features = self.vsa_point_feature_fusion(point_features.view(-1, point_features.shape[-1])) 
        #(2048*batch, 640)-->(2048*batch, 128)

        batch_dict['point_features_vsa'] = point_features  # (BxN, C)
        batch_dict['point_coords_vsa'] = keypoints  # (BxN, 4) 4-->(batch_idx, x, y, z)
        return batch_dict
